...
const createProgressSchema = zod_1.z.object({
    weight: zod_1.z.number().positive('Peso deve ser positivo').optional(),
    bodyFat: zod_1.z.number().min(0).max(100, 'Percentual de gordura deve estar entre 0 e 100').optional(),
    muscle: zod_1.z.number().positive('Massa muscular deve ser positiva').optional(),
    ...
    recordDate: zod_1.z.coerce.date().optional()
});
const updateProgressSchema = createProgressSchema.partial();
const getProgressQuerySchema = zod_1.z.object({
    startDate: zod_1.z.coerce.date().optional(),
    endDate: zod_1.z.coerce.date().optional(),
    limit: zod_1.z.string().transform(val => parseInt(val, 10)).refine(val => !isNaN(val) && val > 0, 'Limit deve ser um número positivo').optional(),
    offset: zod_1.z.string().transform(val => parseInt(val, 10)).refine(val => !isNaN(val) && val >= 0, 'Offset deve ser um número não negativo').optional()
});
class ProgressController {
    ...
    static async create(req, res) {
        try {
            ...
            const progressRecord = await progress_service_1.ProgressService.create(progressData);
            res.status(201).json({
                success: true,
                data: progressRecord,
    ...
    static async getAll(req, res) {
        try {
            ...
            const progressRecords = await progress_service_1.ProgressService.getAll({
                userId: req.user.userId,
    ...
    static async getById(req, res) {
        try {
            ...
            const { id } = req.params;
            const progressRecord = await progress_service_1.ProgressService.getById(id, req.user.userId);
            res.json({
                success: true,
                data: progressRecord
            });
    ...
    static async update(req, res) {
        try {
            ...
            const updatedRecord = await progress_service_1.ProgressService.update(id, req.user.userId, updateData);
            res.json({
                success: true,
                data: updatedRecord,
    ...
    static async delete(req, res) {
        try {
            ...
            const { id } = req.params;
            await progress_service_1.ProgressService.delete(id, req.user.userId);
            res.json({
                success: true,
                data: null,
    ...
    static async getStats(req, res) {
        try {
            ...
            const stats = await progress_service_1.ProgressService.getStats(req.user.userId);
            res.json({
                success: true,
                data: stats
            });
    ...
    static async getChartData(req, res) {
        try {
            ...
            const chartLimit = limit ? parseInt(limit, 10) : 30;
            ...
            const chartData = await progress_service_1.ProgressService.getChartData(req.user.userId, chartLimit);
            res.json({
                success: true,
                data: chartData
            });
    ...
    static async getClientProgress(req, res) {
        try {
            ...
            try {
                const contracts = await contract_service_1.ContractService.getByUserRole(req.user.userId, 'NUTRITIONIST');
                const hasActiveContract = contracts.some(contract => contract.clientId === clientId &&
            ...
            const stats = await progress_service_1.ProgressService.getStats(clientId);
            res.json({
                success: true,
                data: {
