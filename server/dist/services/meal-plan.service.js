...
class MealPlanService {
    ...
    static async getNutritionistProfile(userId) {
    ...
    static validatePlanDates(startDate, endDate) {
    ...
    static validateNutritionalLimits(calories, proteins, limits) {
        if (!limits)
            return;
    ...
    // Criar meal plan
    static async create(data, userId, role, limits) {
    ...
    static async getAll(userId, role, filters = {}, pagination = {}) {
        const whereClause = {};
        if (role === 'NUTRITIONIST') {
            const nutritionistProfile = await this.getNutritionistProfile(userId);
    ...
    static async addMeal(data, userId, role, limits) {
        if (role !== 'NUTRITIONIST') {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (data.foods.length > 0) {
                ...
                if (limits && nutritionalInfo) {
                    this.validateNutritionalLimits(nutritionalInfo.calories, nutritionalInfo.proteins, limits);
                }
            }
    ...
    static async updateMeal(mealId, data, userId, role, limits) {
        if (role !== 'NUTRITIONIST') {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            if (data.suggestedTime !== undefined)
                updateData.suggestedTime = data.suggestedTime;
    ...
    static async copyPlan(options, userId, role) {
        if (role !== 'NUTRITIONIST') {
    ...
    static async recalculatePlanNutrition(mealPlanId, userId, role) {
        if (role !== 'NUTRITIONIST') {
    ...
    static async calculateMealNutrition(mealId, tx) {
    ...
    static async getPlanStatistics(mealPlanId, userId, role) {
        ...
        return {
            ...
            planDuration: Math.ceil((mealPlan.endDate.getTime() - mealPlan.startDate.getTime()) / (1000 * 60 * 60 * 24))
        };
    }
}
