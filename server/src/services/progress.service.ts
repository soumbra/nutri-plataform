import { PrismaClient, ProgressRecord } from '@prisma/client'
...
export class ProgressService {
  ...
  static async create(data: CreateProgressData): Promise<ProgressRecord> {
    try {
  ...
  static async getAll(filters: ProgressFilters): Promise<ProgressRecord[]> {
    try {
      const where: any = {
  ...
  static async getById(id: string, userId: string): Promise<ProgressRecord> {
    try {
  ...
  static async update(id: string, userId: string, data: UpdateProgressData): Promise<ProgressRecord> {
    try {
  ...
  static async getStats(userId: string): Promise<ProgressStats> {
    try {
  ...
  static async getChartData(userId: string, limit: number = 30): Promise<ProgressRecord[]> {
    try {
      const records = await prisma.progressRecord.findMany({
        where: { userId },
