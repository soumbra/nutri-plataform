...
export interface NutritionalSearch {
  ...
  tolerance?: number // percentual de tolerância (ex: 10 = ±10%)
}

export class FoodService {
  // Validar dados nutricionais
  private static validateNutritionalData(data: CreateFoodData | UpdateFoodData) {
  ...
  static async getAll(
    ...
    if (filters.minCalories !== undefined || filters.maxCalories !== undefined) {
      ...
      if (filters.minCalories !== undefined) whereClause.calories.gte = filters.minCalories
      if (filters.maxCalories !== undefined) whereClause.calories.lte = filters.maxCalories
    }

    if (filters.minProteins !== undefined || filters.maxProteins !== undefined) {
      whereClause.proteins = {}
      if (filters.minProteins !== undefined) whereClause.proteins.gte = filters.minProteins
      if (filters.maxProteins !== undefined) whereClause.proteins.lte = filters.maxProteins
    }
  ...
  static async create(data: CreateFoodData, userId: string, role: string) {
  ...
  static async update(id: string, data: UpdateFoodData, userId: string, role: string) {
    if (role !== 'NUTRITIONIST') {
  ...
  static async searchByNutrition(search: NutritionalSearch, pagination: PaginationOptions = {}) {
  ...
  static async getPopular(limit: number = 20) {
